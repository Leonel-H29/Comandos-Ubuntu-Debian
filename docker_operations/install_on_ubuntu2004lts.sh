...
######COMANDOS PARA INSTALAR DOCKER EN UBUNTU 20.04 LTS#####
...
#Este comando descarga la clave de firma GPG desde https://download.docker.com/linux/ubuntu/gpg y luego la agrega 
#al almacén de claves del sistema usando apt-key add -.
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -


#Este comando agrega el repositorio de Docker al sistema. El argumento "deb [arch=amd64] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable" 
#especifica la ubicación del repositorio y utiliza la variable $(lsb_release -cs) para obtener el nombre del código de la versión de Ubuntu actual.
...
#Este comando instala el paquete docker-ce, que es la versión de Docker Community Edition (CE) para Ubuntu. 
#La opción -y se utiliza para confirmar automáticamente cualquier pregunta de confirmación durante la instalación
